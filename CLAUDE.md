...
- **TimesFM 3.0 evaluated and rejected — licensing, not technique (checked 2026-09-01)**: the `v3.0.0` tag exists and `CHECK_FOR_UPDATES` reports it, but the 3.0 weights ship under `timesfm-non-commercial-license-v1.0`, which forbids commercial and production use. The repo source stays Apache-2.0 and **weights up to 2.5 stay Apache-2.0**, so this project stays on TimesFM 2.5: a demand forecast driving procurement is precisely the excluded use. Two facts worth not rediscovering. First, `v3.0.0` still ships the 2.5 package intact — the `v2.0.2 → v3.0.0` diff over `src/timesfm/` is 9 lines, one of them a `force_flip_invariance` fix that only executes when `max_horizon > 128`, which with `HORIZON = 24` it never does; moving the pin would be numerically neutral and pointless. Second, TimesFM-3's new capabilities (native multivariate, past/future covariates, 15k context, non-autoregressive decoding) are unusable here: 571 univariate monthly series, 30-60 points each, no covariates available. Reopen only if the 3.0 weights go Apache-2.0 or a commercial licence becomes purchasable
...
| Measured outcome (2026-08-27) | Inference **289.8 s → 9.4 s (30.7x)** locally, **195.0 s → 13.0 s (15.0x)** on a Colab T4; whole run 5m13s → 49 s. Numbers unchanged: the v1.5.1→v1.6.0 refactor is bit-identical, batch 1 vs 32 moves nothing at all with the backtest on, and exactly **one cell out of 13,704** without it (one pack, on a value sitting on the rounding midpoint). Local and Colab agree bit-for-bit at the same batch size. Full table in the README's "Prestazioni misurate" |
